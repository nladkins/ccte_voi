   ...
   loadedData <- reactiveValues(
    ...
    if (file.exists("DefaultScenarios.xlsx"))
       {
         ...
         # shinyjs::disable("downloadComputedResult")
          ...
          if (isolate(loadedData$defaultCount) > 0)
          {
            ...
            observe({
                  ...
                  else
                  {
                    ...
                    messageText <- ""
                    scenarioID <- colID - 11 # this value should be [colID - (number of default scenarios - 1)]
    ...
    output$meanThresholdDistribution <- renderPlot({
      if (!is.na(input$muMeanThreshold))
        ...
        plot(seq(xlimit[1], xlimit[2], 0.01), dnorm(seq(xlimit[1], xlimit[2], 0.01), input$muMeanThreshold, u.sd(input$sigmaMeanThreshold, range=0.9995)), xlim=xlimit, main = bquote(Prior~Uncertainty~Distribution~About~log[10](ED[50])), xlab = bquote(log[10](ED[50])), ylab = "Density", type = "n")
        grid()
    ...
    output$meanExposureDistribution <- renderPlot({
      if (!is.na(input$muMeanExposure))
        xlimit=c(min(input$muMeanExposure, input$muMeanThreshold)-5, max(input$muMeanExposure, input$muMeanThreshold)+5)
        plot(seq(xlimit[1], xlimit[2], 0.01), dnorm(seq(xlimit[1], xlimit[2], 0.01), input$muMeanExposure, u.sd(input$sigmaMeanExposure, range=0.9995)), xlim=xlimit, main = bquote(Prior~Uncertainty~Distribution~About~log[10](Mean~Exposure)), xlab = bquote(log[10](Mean~Exposure)), ylab = "Density", type = "n", col=2)
        grid()
    ...
    output$sdr <- renderPlot({
      if (input$sdr>=0){
        ...
        plot(seq(y.start, y.end+5), disc.rate.vec*100, main = plot.title, ylim=c(0,100), xlab = "Year", ylab = "Present value of a future cost of 100 USD", type = "n")
        grid()
                  ...
                  if (variableValue$bolUserFileLoaded == FALSE)
                     {
                       ...
                       if(ext == "xlsx")
                       {
                         ...
                         if (variableValue$fileUUID != "") #it could be created when user created scenario using default scenario
                         {
                           ...
                           tempFile[[1]] <- NULL #delete first column 
                         ...
                         shinyjs::enable("downloadComputedResult")
                       }
                       else
                       {
                         output$errorMessage0 <- renderText({
    ...
    output$downloadComputedResult <- downloadHandler(
      
      filename =  "VOI Analysis Output.html",
      ...
      content = function(file) {
        ...
        if(as.numeric(input$decisionRule)==2){
          if(as.numeric(input$sigmaMeanExposure)==0){
            if(as.numeric(input$RS_included)!=2){
              rmarkdown::render("Code/Rmd/epavoi_TRDM_SC1.Rmd", output_file = file, params = params)
            }

            if(as.numeric(input$RS_included)==2){
            rmarkdown::render("Code/Rmd/epavoi_TRDM_SC1_NoRS.Rmd", output_file = file, params = params)
            }
          }
          if(as.numeric(input$sigmaMeanExposure)>0){
            if(as.numeric(input$RS_included)!=2){
              rmarkdown::render("Code/Rmd/epavoi_TRDM_SC3.Rmd", output_file = file, params = params)
            }
            if(as.numeric(input$RS_included)==2){
              rmarkdown::render("Code/Rmd/epavoi_TRDM_SC3_NoRS.Rmd", output_file = file, params = params)
            }
        ...
        if(as.numeric(input$decisionRule)==1){
          if(as.numeric(input$sigmaMeanExposure)==0){
            if(as.numeric(input$RS_included)==2){
              rmarkdown::render("Code/Rmd/epavoi_BRDM_SC1_NoRS.Rmd", output_file = file, params = params)
            }
            if(as.numeric(input$RS_included)!=2){
              rmarkdown::render("Code/Rmd/epavoi_BRDM_SC1.Rmd", output_file = file, params = params)
            }
          }
          if(as.numeric(input$sigmaMeanExposure)>0){
            if(as.numeric(input$RS_included)!=2){
              rmarkdown::render("Code/Rmd/epavoi_BRDM_SC3.Rmd", output_file = file, params = params)
            }
            if(as.numeric(input$RS_included)==2){
              rmarkdown::render("Code/Rmd/epavoi_BRDM_SC3_NoRS.Rmd", output_file = file, params = params)
            }
          }
        }
