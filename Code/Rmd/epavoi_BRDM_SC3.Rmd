...
```{r IPPI}
if (shiny::isRunning()){ params$progress$set(value=0.58) }
#------------#
# For EVIPPI #
#------------#
...
for (i1 in 1:length(u.mu.tox.posterior.prop.vec)){
  ...
  sigma_expr.A=sigma_expr_function(u.mu.tox.posterior.A, u.mu.tox.prior) # For test A
  # print(c(u.mu.tox.posterior.A, sigma_expr.A))
  ...
  for (tt in t1.seq){
    ...
    # print(c(EV.CI, u.mu.tox.posterior.A, tt, EVISI.A))
  }
...
EVDSI.mat.SC2=rbind(EVDSI.mat, EVCI.vec)
# print(EVDSI.mat.SC2)
...
```{r BRDM_Response_Surface_Output}
if (shiny::isRunning()){ params$progress$set(value=0.92) }
library(htmlwidgets)
library(plotly)
...
fig <- fig %>% add_surface(y=~Uncertainty_Reduction, x=~Delay, z = ~EVDSI) %>%
  ...
  add_trace(x=t.A, y=100*(1-u.sd(A.range, u.range)/u.mu.tox.prior.true), z=EVDSI.A, type="scatter3d", mode="markers", marker.color="red", symbols='x', name="Test A") %>%
  add_trace(x=t.B, y=100*(1-u.sd(B.range, u.range)/u.mu.tox.prior.true), z=EVDSI.B, type="scatter3d", mode="markers", name="Test B")
fig
