---
title: "Results for BRDM: Uncertainty present only in mean toxicity"
...
# Number of replications (B - sample size for prior uncertainty distribution)
LP1=10000
...
```{r prior}
if (shiny::isRunning()){ params$progress$set(value=0.17) }
#---------------------------#
...
k.equal.0.PTI.vec=loc.kstar.prior.vec=NULL
for (ind in 1:dim(TSC.all)[1]){
  loc.kstar.prior=which(TSC.all[ind,]==min(TSC.all[ind,]))
  loc.kstar.prior.vec=c(loc.kstar.prior.vec, loc.kstar.prior)
  if(loc.kstar.prior>1){k.equal.0=0}
  if(loc.kstar.prior==1){k.equal.0=1}
  k.equal.0.PTI.vec=c(k.equal.0.PTI.vec, k.equal.0)
}
PD1.PTI=1-sum(k.equal.0.PTI.vec*pdf)
...
```{r posterior A pt1}
if (shiny::isRunning()){ params$progress$set(value=0.26) }
#------------------------#
...
if (shiny::isRunning()){ params$progress$set(value=0.35) }
...
```{r posterior B pt1}
if (shiny::isRunning()){ params$progress$set(value=0.43) }
#------------------------#
...
```{r posterior B pt2}
if (shiny::isRunning()){ params$progress$set(value=0.52) }
...
```{r output AB}
if (shiny::isRunning()){ params$progress$set(value=0.61) }
#--------#
# OUTPUT #
#--------#
...
VOI.summary=rbind(c(k.opt.perc, rep(NA, 3)), c(EV.CI.fmt, rep(NA, 3)), c(EVIPI.fmt, rep(NA, 3)),
...
rownames(VOI.summary)=c("k0", "EV|CI", "EVIPI", "EVISI", "CoD", "EVDSI", "ENBS", "ROI")

# Formatting VOI summary information (EV|CI and EVIPPI)
VOI.summary.prior.information=rbind(c(NA, k.opt.perc), c(EV.CI.fmt, NA), c(EVIPI.fmt, NA))
colnames(VOI.summary.prior.information)=c("(&#x0024;M)", "(&#x0025;)")
rownames(VOI.summary.prior.information)=c("Optimal Reduction in Exposure (ORE)", "Expected Value Given Current Information (EV|CI)", "Expected Value of Immediate Perfect Information (EVIPI)")
kable(VOI.summary.prior.information, caption="VOI summary (part 1) - Current information and immediate perfect information", align = rep("r", 2), format.args = list(big.mark = ","))
...
```{r BRDM_EVDSI_Table_Computation_1}
if (shiny::isRunning()){ params$progress$set(value=0.70) }
EV.IPI.vec=EV.CI.a.vec=NULL
for(t1 in t1.seq){
  t.IC=t1
  ...
  ETSC.star.vec=apply(TSC.all.weighted, 1, min) # A vector in which the average equals the EV|PI
  EV.IPI.a=sum(ETSC.star.vec)
  
  EV.IPI.vec=c(EV.IPI.vec, EV.IPI.a)
  EV.CI.a.vec=c(EV.CI.a.vec, EV.CI.a)
}
...
```{r BRDM_EVDSI_Table_Computation_2}
if (shiny::isRunning()){ params$progress$set(value=0.78) }
#---------------#
# For Posterior #
#---------------#
...
for (i1 in 1:length(u.mu.tox.posterior.prop.vec)){
  # Attempting to obtain unconditional pdf for sample information s_j
  s.j.vec=seq(-bounds, bounds, by=u.range.spacing)*u.mu.tox.prior+mu.tox.prior # this is wider than the original uncertainty range to ensure the chance s_j can be more extreme
  ...
  for (tt in t1.seq){
    ...
    EV.ISI.A=sum(ETSC.ISI.A.vec*f.sj.vec)
    EVISI.A=EV.CI-EV.ISI.A
    EVDSI.vec=c(EVDSI.vec, EVISI.A)
    # print(c(EV.CI, EV.ISI.A, EVISI.A))
  }
  
  EVDSI.mat=rbind(EVDSI.mat, EVDSI.vec)
...
EVIPI.vec=EV.CI-EV.IPI.vec
EVCI.vec=EV.CI-EV.CI.a.vec
EVDSI.mat.SC2=rbind(EVIPI.vec, EVDSI.mat, EVCI.vec)

rownames(EVDSI.mat.SC2)=rownames.EVDSI.mat
colnames(EVDSI.mat.SC2)=colnames.EVDSI.mat
...
```{r plot_RS}
if (shiny::isRunning()){ params$progress$set(value=0.87) }

library(htmlwidgets)
library(plotly)
EVDSI.1orig=EVDSI.mat.SC2
z1orig=EVDSI.mat.SC2
EVDSI=z1orig
...
kable(round(EVDSI.mat.SC2), caption="EVDI as a function of testing time and uncertainty reduction", format.args = list(big.mark = ","))
```
...
```{r Outputting_Input}
if (shiny::isRunning()){ params$progress$set(value=0.96) }
